...
bool timeDayNoon = false;
bool sendNotify = false;

void RunScriptEveryFrame()
{
	if (timeDayNoon)
	{
	...
	if (sendNotify)
	{
		NotifyStatic(TEX_ICON_SYSTEM, "Enabled sendNotify");
		sendNotify = false;
	}
}
...
void SelfOptions()
{
	menu.option("Set time of day noon").toggle(&timeDayNoon);
	menu.option("Send system notification").toggle(&sendNotify);
}

void mainMenu()
{
	menu.banner("Main Menu");
	menu.option("Example").submenu(SelfOptions);
}
